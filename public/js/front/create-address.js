//新建收货地址结果
function createShippingAddressOk(data)
{
...
function showPosition(position)
{
	...
	geoc.getLocation(point, function(rs){
		...
		//$('.choose-title').text(addComp.street+addComp.streetNumber);
...
$(document).ready(function(){
    ...
    // 收货人
    $('.edit-item-title-name').click(function(){
    	$('.edit-name-box input').focus();
    });

    //联系电话
	$('.edit-item-title-phone').click(function(){
    	$('.edit-phone-box input').focus();
    });

    // 详细地址
    $('.edit-item-title-address').click(function(){
    	$('.edit-concret-address-box input').focus();
    });
				...
				$.ajax({
					...
					success: function(data){
						if(data['status']['success']){
				    		...
				    		sendAjax(postArr, '/api/v1/user/createShippingAddress', 'false', 'createShippingAddressOk');
						} else {
							//成功  跳转重新登录
							$.ajax({
								url: "/api/v1/front/userOut",
