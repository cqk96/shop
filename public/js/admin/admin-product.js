...
function judgeSubmit()
{
	...
	if($('.each-sku-box').length==0){
		...
		// 存在sku
		var inventoryOk = true,priceOk = true,commission = true;
		var position = 0;
		$("input[name='inventory[]']").each(function(key){
		...
		if(!inventoryOk){$("input[name='inventory[]']").eq(position).focus();return false;}
		
		$("input[name='price[]']").each(function(key2){
			if($(this).val()==""||parseFloat($(this).val())==0){
				position = key2;
				priceOk = false;
				return false;
		...
		if(!priceOk){$("input[name='price[]']").eq(position).focus();return false;}

		$("input[name='priceCommission[]']").each(function(key3){
			if(parseFloat($(this).val())>=parseFloat($("input[name='price[]']").eq(key3).val())){
				position = key3;
				commission = false;
				return false;
			}
		...
		if(!commission){$("input[name='priceCommission[]']").eq(position).focus();return false;}
...
// 移除sku节点
function removeAttrNode(obj)
{
	var format = /doUpdate/;
	if(format.test($("#myForm").prop("action"))){

		var format2 = /input/;

		// 判断是否是input节点
		if(format2.test($(obj).prev()[0]['localName'])){
			// 动态删除sku	
			var postArr = new Array();
			postArr['id'] = $(obj).prev().val();
			sendAjax(postArr,'/admin/product/doDeleteSku', 'true');
		}
	}
	$(obj).parent().remove();
}
