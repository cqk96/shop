...
class LikeChatCircleModel extends BaseModel {
	...
	{
		$this->_model = new \EloquentModel\LikeChatCircle; 
	}
	...
	*/
	public function lists()
	{
		...
		//设置页数跳转地址
		$pageObj->setUrl('/admin/likeChatCircles');
		// 设置分页数据
	...
	* @return 	array
	*/
	public function getLikedRecord($chatId, $uid)
	{
		return $this->_model->where("chat_id", $chatId)->where("user_id", $uid)->first();
	}
	...
	* @return 	array
	*/
	public function getUserLikeChats($uid)
	{
		return $this->_model->where("user_id", $uid)->where("is_deleted", 0)->select("chat_id")->get()->toArray();
	}
