...
class TenDayDiaryCommentModel extends BaseModel {
	...
	{
		$this->_model = new \EloquentModel\TenDayDiaryComment; 
	}
	...
	*/
	public function lists()
	{
		...
		//设置页数跳转地址
		$pageObj->setUrl('/admin/tenDayDiaryComments');
		// 设置分页数据
	...
	* @param 	type 			$typeId    	diary comment type id
	* @return 	string
	*/
	public function getDiaryCommentContentWithTypeId($id, $typeId)
	{
		$data = $this->_model->where("is_deleted", 0)->where("diary_id", $id)->where("comment_type_id", $typeId)->select("evaluation")->take(1)->get()->toArray();
		return empty($data)? '':$data[0]['evaluation'];
	}
	...
	*/
	...
	{
		$count = $this->_model->where("is_deleted", 0)->where("diary_id", $id)->count();
