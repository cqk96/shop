...
class ProductTemplateManagementModel extends BaseModel {
	...
	{
		$this->_model = new \EloquentModel\ProductTemplateManagement; 
	}
	...
	*/
	...
	// 	//设置页数跳转地址
	// 	$pageObj->setUrl('/admin/productTemplateManagements');
	// 	// 设置分页数据
	...
	*/
	public function getListsObject($params=[])
	{
		...
		$query = $this->_model->select("id", "name", "country_id", "language_id", "update_time as updateTime")
							  ->where("is_deleted", 0)
							  ->orderBy("id", "desc");
	...
	* @return 	array
	*/
	public function searchTemplateObjWithParams( $params=[])
	{
		
		// 分页对象
		$pageObj = new \VirgoUtil\Page2;

		$query = $this->_model->select("id", "name", "template_url", "cover")->where("is_deleted", 0)->orderBy("id", "desc");
		...
		if( !empty( $params['language_id'] ) ) {
			$query = $query->where("language_id", $params['language_id']);
		}
