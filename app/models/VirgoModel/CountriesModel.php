...
class CountriesModel extends BaseModel {
	...
	{
		$this->_model = new \EloquentModel\Countries; 
	}
	...
	*/
	...
	// 	//设置页数跳转地址
	// 	$pageObj->setUrl('/admin/countries');
	// 	// 设置分页数据
	...
	*/
	public function getListsObject($params=[])
	{
		...
		$query = $this->_model->select("id", "name", "update_time as updateTime")
							  ->where("is_deleted", 0)
							  ->orderBy("id", "desc");
	...
	* @return 	string
	*/
	public static function getCountryNameWithId( $countryId ) 
	{

		$countryArr = ['中国台湾', '越南', '新加坡', '泰国', '柬埔寨', '马拉西亚', '印度尼西亚', '菲律宾'];

		if( empty( $countryArr[ intval($countryId)-1] ) ){
			return '未知';
		}

		return $countryArr[ intval($countryId)-1];
