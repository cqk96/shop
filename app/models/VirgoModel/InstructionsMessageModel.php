...
class InstructionsMessageModel extends BaseModel {
	...
	{
		$this->_model = new \EloquentModel\InstructionsMessage; 
	}
	...
	*/
	public function lists()
	{
		...
		//设置页数跳转地址
		$pageObj->setUrl('/admin/instructionsMessages');
		// 设置分页数据
	...
	* @return 	array
	*/
	public function getCreatorInstructionLists($uid, $skip=null, $size=null)
	{
		
		$query = $this->_model->leftJoin("users", "users.id",'=', 'instructions_message.author_id')
							->where("instructions_message.is_deleted", 0)
							->where("users.is_deleted", 0)
							->where("instructions_message.author_id", $uid)
							->orderBy("instructions_message.create_time", "desc")
							->select("instructions_message.id", "content", "instructions_message.create_time as createTime", "name");
