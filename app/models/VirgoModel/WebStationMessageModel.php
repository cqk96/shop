...
class WebStationMessageModel extends BaseModel {
	...
	{
		$this->_model = new \EloquentModel\WebStationMessage; 
	}
	...
	*/
	public function lists()
	{
		...
		// set query 
		$query = $this->_model->leftJoin("users", "users.id", "=", "web_station_message.author_id")
 							  ->where("users.is_deleted", '=', 0)
 							  ->where("web_station_message.is_deleted", '=', 0)
 							  ->orderBy("create_time", "desc")
 							  ->select("web_station_message.*", "users.name");
		...
		//设置页数跳转地址
		$pageObj->setUrl('/admin/webStationMessages');
		// 设置分页数据
