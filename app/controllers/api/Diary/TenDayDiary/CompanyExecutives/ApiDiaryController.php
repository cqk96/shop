<?php
namespace VirgoApi\Diary\TenDayDiary\CompanyExecutives;
...
class ApiDiaryController extends \VirgoApi\ApiBaseController{
	...
	*/
	...
	/**
	* @SWG\Post(path="/api/v1/diary/tenDayDiary/companyExecutives/save", tags={"TenDayDiary"}, 
	*  summary="公司高管审批",
	*  description="用户鉴定后 通过传入的指定参数进行公司高管审批 修改行为也为同一个接口 参数不同",
	...
	public function save()
	{
		...
		try{
			...
			if( !empty($record) ) {
				...
				$commentInsertData['comment_type_id'] = 2;
				...
				if( !$recordId ) {
					throw new \Exception("创建公司高管审批失败", '005');
				}
			...
			$return = $this->functionObj->toAppJson($recordId, '001', $message . '成功', true);
