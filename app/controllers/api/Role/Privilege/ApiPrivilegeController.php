<?php
namespace VirgoApi\Role\Privilege;
use VirgoApi;
class ApiPrivilegeController extends VirgoApi\ApiBaseController{
	...
	*/
	...
	/**
	* @SWG\Get(path="/api/v1/role/privilege/inAll", tags={"Role"}, 
	*  summary="获取角色有的权限",
	*  description="鉴权后 通过传入的角色id 获取权限列表  有标记字段标识是否有此权限",
	...
	*   response=200,
	*   description="操作成功",
	*   examples={"application/json": { "data": "RolePrivilegeLists", "status": { "code": "001", "message": "获取角色权限列表成功", "success": true } } },
	*   @SWG\Schema(
	*    type="object",
	*    ref="#/definitions/RolePrivilegeLists"
	*   )
	*  )
	* )
	* 获取角色有的权限
	* 鉴权后 通过传入的角色id 获取权限列表  有标记字段标识是否有此权限
	* @author 	xww
	* @return 	json
	*/
	public function inAll()
	{
		...
		try{
			...
			*/
			...
			$data = $model->getRolePrivileges($id);
			$data = empty($data)? null:$data;

			$return = $this->functionObj->toAppJson($data, '001', '获取角色权限列表成功', true);
	...
	/**
	* @SWG\Post(path="/api/v1/role/privilege/update", tags={"Role"}, 
	*  summary="更新角色权限",
	*  description="传入令牌和账号进行鉴权 鉴权后 通过传入的角色id 和权限字符串以,分隔的权限id进行创建 通过标识doEmpty来规定是否执行清空角色权限操作",
	*  produces={"application/json"},
	...
	*   response=200,
	*   description="操作成功",
	*   examples={"application/json": { "data": null, "status": { "code": "001", "message": "更新角色权限成功", "success": true } } }
	*  )
	* )
	* 
	* 更新角色权限
	* 传入令牌和账号进行鉴权 鉴权后 通过传入的角色id 和权限字符串以,分隔的权限id进行创建 通过标识doEmpty来规定是否执行清空角色权限操作
	* @author 	xww
	* @return 	json
	*/
	public function update()
	{
		...
		try{
			...
			// 角色权限对象
			$model = new \VirgoModel\PrivilegeToRoleModel;
			...
			*/
			...
			// 权限ids
			$privilegeIds = explode(",", $this->_configs['ids']);

			// 角色id
			$roleId = (int)$this->_configs['roleId'];

			$rs = $model->saveRolePrivileges($roleId, $privilegeIds, $doEmpty);

			if( !$rs ) {
				throw new \Exception("更新角色权限失败", '003');
			}

			$return = $this->functionObj->toAppJson(null, '001', '更新角色权限成功', true);
