	...
	<tbody>
		...
		<tr class="<?php echo $i%2==0? 'singular':'dual' ?>">
			...
			<td class='operationBox' width='20%'>
				<a href="/admin/sys/ptr/update?id=<?php echo $data[$i]['role_id']; ?>" ><span class="icon-img"><img src="/images/edit-icon.png" /></span>修改</a>
				<a href="/admin/sys/ptr/doDelete?id=<?php echo $data[$i]['role_id']; ?>" onclick="return confirm('你确定要删除吗？')" ><span class="icon-img"><img src="/images/delete-icon.png" /></span>删除</a>
			</td>
		</tr>
		<?php } ?>
	</tbody>
...
$(document).ready(function(){
	...
	$('.deleteChooseBtn').click(function(){
		deleteChooseItems('ids','你确定要删除选定项吗？','/admin/sys/ptr/destroy','true','showDeleteItems');
	});
});
</script>