<?php include_once('../app/views/admin/_header.php') ?>
 ...
 <div class='smart-widget'>
	 <div class='smart-widget-header'>
		...
		<form class='searchForm' action='farms' method='get'>
	 ...
	 <table id='indexTable' class=''>
		 ...
		 <tbody>
			 ...
			 <tr class='<?php echo $i%2==0? 'singular':'dual' ?>'>
				...
				<td><?php echo $data[$i]['acre_amount'];?></td>
				<td><?php echo $data[$i]['create_time'];?></td>
				<td><?php echo $data[$i]['update_time'];?></td>
				
				 <td class='operationBox' colspan=2>
					 <a href='/admin/farms/update?id=<?php echo $data[$i]['id']; ?>' ><span class='icon-img'><img src='/images/edit-icon.png' /></span>编辑</a>
					 <a type='button' href='/admin/farms/doDelete?id=<?php echo $data[$i]['id']; ?>' onclick='return confirm("你确定要删除吗？")'><span class='icon-img'><img src='/images/delete-icon.png'></span>删除</a>
				 </td>
			 </tr>
			 <?php } ?>
		 </tbody>
		 <?php include_once('../app/views/admin/adminFarm/_tfoot.php') ?>
	 </table>
 </div><!-- ./smart-widget -->
 </div>
 ...
 $(document).ready(function(){
	 ...
	 $('.deleteChooseBtn').click(function(){
		 deleteChooseItems('eachNewsClassCheckBox','你确定要删除选定项吗？','/admin/farms/destroy','','showDeleteItems','');
	 });
	//日期选择
