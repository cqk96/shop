...
<div class="smart-widget">
	...
	<tbody>
		...
		<tr class="<?php echo $i%2==0? 'singular':'dual' ?>">
			<td width='10%'><input type="checkbox" name="ids" class='ids' value="<?php echo $data[$i]['user_id'] ?>"></td>
			<td width='10%'><?php echo ($i+1);?></td>
			<td width='20%'> <?php echo empty($users[$data[$i]['user_id']])? '用户被删除':$users[$data[$i]['user_id']]['user_login']; ?> </td>
			<td width='30%'> <?php echo $data[$i]['user_roles']; ?> </td>
			<td class='operationBox' width='20%'>
				<a href="/admin/sys/rtu/update?id=<?php echo $data[$i]['user_id']; ?>" ><span class="icon-img"><img src="/images/edit-icon.png" /></span>修改</a>
				<a href="/admin/sys/rtu/doDelete?id=<?php echo $data[$i]['user_id']; ?>" onclick="return confirm('你确定要删除吗？')" ><span class="icon-img"><img src="/images/delete-icon.png" /></span>删除</a>
			</td>
		</tr>
		<?php } ?>
	</tbody>
...
$(document).ready(function(){
	...
	$('.deleteChooseBtn').click(function(){
		deleteChooseItems('ids','你确定要删除选定项吗？','/admin/sys/rtu/destroy','true','showDeleteItems');
	});
});
</script>