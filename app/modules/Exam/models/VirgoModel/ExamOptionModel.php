...
use VirgoModel;
class ExamOptionModel extends VirgoModel\BaseModel {
	...
	{
		$this->_model = new \Module\Exam\EloquentModel\ExamOption; 
	}
	...
	*/
	public function lists()
	{
		...
		//设置页数跳转地址
		$pageObj->setUrl('/admin/examOptions');
		// 设置分页数据
	...
	*/
	...
	/**
	* 根据questionId和 题目选项索引 获取选项
	* @author 	xww
	* @param 	int/string 		$questionId
	* @param 	int/string 		$index
	* @return 	array
	*/
	public function getExamQuestionOptionIndex($questionId, $index)
	{
		return $this->_model->where("is_deleted", 0)->where("eq_id", $questionId)->where("option_index", $index)->first();
	}
	/**
	* 创建记录
	* @author 	xww
	...
	*/
	...
	/**
	* 根据题目id 和选项缩影 获取选项（不管是否删除）
	* @author 	xww
	* @param 	int/string 		$questionId
	* @param 	int/string 		$index
	* @return 	array
	*/
	public function getExamQuestionOption($questionId, $index)
	{
		return $this->_model->where("eq_id", $questionId)->where("option_index", $index)->first();	
	}
